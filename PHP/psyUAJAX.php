...
if(count($tabID) ==0)
{
	...
	echo "<button id='psyAktualizujPowrotBrak' onclick='openPsyCRUD(event)'> Powrót </button>";
}
else
{
	echo "Te rekordy zostaną zmodyfikowane, czy na pewno chcesz kontynuować? <br/>";
	echo "<button id='psyAktualizujPowrot' onclick='openPsyCRUD(event)'> Nie! </button>";
	echo "<button id='psyAktualizujDalej' onclick='openPsyUDalej(event)'> Tak! </button>";
	$IDs = "" . $tabID[0];
	for($var = 1; $var<count($tabID); $var++)
	{
		$IDs = $IDs . "," . $tabID[$var];
	}
	echo "<input type='text' name='ID' class = 'ukryte' id='psyUkryteIDU' value=$IDs readonly/>";
}
?>
