...
if(count($tabID) ==0)
{
	...
	echo "<button id='przewodnicyAktualizujPowrotBrak' onclick='openPrzewodnicyCRUD(event)'> Nie! </button>";
}
else
{
	echo "Te rekordy zostaną zmodyfikowane, czy na pewno chcesz kontynuować? <br/>";
	echo "<button id='przewodnicyAktualizujPowrot' onclick='openPrzewodnicyCRUD(event)'> Nie! </button>";
	echo "<button id='przewodnicyAktualizujDalej' onclick='openPrzewodnicyUDalej(event)'> Tak! </button>";
	$IDs = "" . $tabID[0];
	for($var = 1; $var<count($tabID); $var++)
	{
		$IDs = $IDs . "," . $tabID[$var];
	}
	echo "<input type='text' name='ID' id='ukryteID' value=$IDs readonly/>";
}
?>
